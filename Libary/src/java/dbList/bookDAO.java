...
public class bookDAO {
    ...
    public String getbook_name(int book_id){
        ...
        try{
            Connection con = db.getConnection();
            String sql = "select top 1 * from book_copy bc left join book b on bc.book_id=b.id where bc.id = '" + book_id +"'";
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery(sql);
            if (rs.next()){
    ...
    public boolean getstatus(String book_name){
        System.out.println("dbList.bookDAO.getstatus()"+book_name);
        DBContext db = new DBContext();
        try{
            Connection con = db.getConnection();
            String sql = "select * from book_copy bc left join book b on bc.book_id=b.id where is_returned=0 and b.title = '" + book_name +"'";
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery(sql);
            if (rs.next()){
                    return true;
